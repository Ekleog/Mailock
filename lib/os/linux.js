...
function run(args, stdin) {
   ...
   var ret = { out: "", err: "" };
   if (!args.stdin)
      args.stdin = subprocess.WritablePipe(function() {
         this.write(stdin);
      });
   if (!args.stdout)
      args.stdout = subprocess.ReadablePipe(function(data) {
         ret.out = data;
      });
   if (!args.stderr)
      args.stderr = subprocess.ReadablePipe(function(data) {
         ret.err = data;
      });
   if (!args.command)
      args.command = os.inPath("gpg2");
   if (!args.environment)
      args.environment = ["LANG=C"];
   subprocess.call(args).wait();
   return ret;
}
