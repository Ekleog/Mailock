...
function run(args, stdin, no_keyserver) {
   if (!stdin) stdin = "";
   // Sanitize input
   ...
   if (!no_keyserver)
      cmd += "--keyserver " + pgp_utils.random_keyserver() + " ";
   cmd += "> " + stdout + " 2> " + stderr;
...
exports.handshake = function(other, check_emails, info) {
   ...
   // Run command
   run(["--command-fd=0", "--batch", "--edit-key", pgp_utils.keyid(other)], cmd, true);
...
exports.decrypt = function(msg, from) {
   ...
   if (parse.check_signature(ret.err, from)) return { msg: ret.out, ok: true  };
   else                                      return { msg: ret.out, ok: false };
}
