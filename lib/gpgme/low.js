...
function bitfield_get(obj, bitfield, bit, name) {
   obj[name] = (bitfield >> bit & 1) ? true : false; // TODO: Check it's valid
}
// }}}
...
function engine_info_t(next, protocol, file_name, home_dir,
                       version, req_version) {
   this.next        = engine_info_t_from_ptr(next);
...
exports.data_release_and_get_mem = function(data) {
   ...
   if (size.toSource().substr(21, 6) == "UInt64")
      size = ctypes.UInt64.lo(size.value);
   let buffer = ArrayBuffer(size);
   let view = Uint8Array(buffer);
   let iter = ptr;
   for (let i = 0 ; i < size ; ++i, iter = iter.increment())
      view[i] = iter.contents;
   ns.free(ptr);
   return buffer;
}
// }}} 6.2
...
// }}} 7.2
// {{{ 7.3
decl("result_ref", ctypes.void_t, ctypes.voidptr_t);
decl("result_unref", ctypes.void_t, ctypes.voidptr_t);
// }}} 7.3
// {{{ 7.4
// {{{ 7.4.1
...
// }}} 7.4.4
// {{{ 7.4.5
exports.include_certs = {};
exports.include_certs.DEFAULT = -256;
...
// TODO: Test (on next gpgme release, as there was a bug fix)
...
function subkey_t(next, _bitfield, pubkey_algo, length, keyid, fpr, timestamp,
                  expires, card_number) {
   this.next        = subkey_t_from_ptr(next);
   ...
   bitfield_get(this, _bitfield,  0, "revoked");
   bitfield_get(this, _bitfield,  1, "expired");
   bitfield_get(this, _bitfield,  2, "disabled");
   bitfield_get(this, _bitfield,  3, "invalid");
   bitfield_get(this, _bitfield,  4, "can_encrypt");
   bitfield_get(this, _bitfield,  5, "can_sign");
   bitfield_get(this, _bitfield,  6, "can_certify");
   bitfield_get(this, _bitfield,  7, "secret");
   bitfield_get(this, _bitfield,  8, "can_authenticate");
   bitfield_get(this, _bitfield,  9, "is_qualified");
   bitfield_get(this, _bitfield, 10, "is_cardkey");
}
// }}}
...
function sig_notation_t(next, name, value, name_len, value_len, flags,
                        _bitfield) {
   this.next      = sig_notation_t_from_ptr(next);
   this.name      = name;
   this.value     = value;
   ...
   bitfield_get(this, _bitfield, 0, "human_readable");
   bitfield_get(this, _bitfield, 1, "critical");
}
// }}}
// {{{ key_sig_t
...
function key_sig_t(next, _bitfield, pubkey_algo, keyid, timestamp, expires,
   ...
   this.next        = key_sig_t_from_ptr(next);
   this.pubkey_algo = pubkey_algo;
   this.keyid       = keyid;
   this.timestamp   = timestamp;
   this.expires     = expires;
   ...
   this.notations   = sig_notation_t_from_ptr(notations);

   bitfield_get(this, _bitfield, 0, "revoked");
   bitfield_get(this, _bitfield, 1, "expired");
   bitfield_get(this, _bitfield, 2, "invalid");
   bitfield_get(this, _bitfield, 3, "exportable");
}
// }}}
// {{{ validity_t
...
function user_id_t(next, _bitfield, validity, uid, name, email, comment,
                   signatures) {
   this.next       = user_id_t_from_ptr(next);
   this.validity   = validity;
   this.uid        = uid;
   this.name       = name;
   this.email      = email;
   this.comment    = comment;
   this.signatures = key_sig_t_from_ptr(signatures);

   bitfield_get(this, _bitfield, 0, "revoked");
   bitfield_get(this, _bitfield, 1, "invalid");
}
// }}}
// {{{ key_t
...
function key_t(_bitfield, protocol, issuer_serial, issuer_name, chain_id,
   ...
   this.subkeys       = subkey_t_from_ptr(subkeys);
   this.uids          = user_id_t_from_ptr(uids);
...
exports.op_keylist_next = function(ctx) {
   ...
   if (ret != err.code.NO_ERROR) {
      return { ret: ret, res: null };
   } else {
      let key = key_t_from_ptr(res);
      ns.key_unref(res);
      return { ret: ret, res: key };
   }
}
...
function keylist_result_t(struct) {
   bitfield_get(this, struct._bitfield, 0, "truncated");
}

decl("op_keylist_result", keylist_result_t_ptr, ctx_t);
exports.op_keylist_result = function(ctx) {
   return keylist_result_t(ns.op_keylist_result(ctx).contents);
}
...
decl("key_unref", ctypes.void_t, key_t_ptr);
// Deprecated interfaces not included
// }}} 7.5.4
// }}} 7.5
// }}} 7
