...
// TODO: Find a better way to manage arrays
...
decl("data_new_from_mem",
      error_t,
      data_t.ptr, string, ctypes.size_t, ctypes.int);
exports.data_new_from_mem = function(data, copy) {
   let res = data_t();
   let ret = ns.data_new_from_mem(res.address(), data, data.length, copy ? 1 : 0);
   return { ret: ret, res: res };
}
...
decl("data_release_and_get_mem", ctypes.char.ptr, data_t, ctypes.size_t.ptr);
exports.data_release_and_get_mem = function(data) {
   let res = ctypes.size_t();
   let ret = ns.data_release_and_get_mem(data, res.address());
   return { ret: ret, res: res };
}

decle("free", ctypes.void_t, ctypes.voidptr_t);
// }}} 6.2
// {{{ 6.3
// {{{ 6.3.1
decle("data_read",  ctypes.ssize_t, data_t, ctypes.voidptr_t, ctypes.size_t);
decle("data_write", ctypes.ssize_t, data_t, ctypes.voidptr_t, ctypes.size_t);
decle("data_seek",  ctypes.ssize_t, data_t, off_t, ctypes.int);
// }}} 6.3.1
// {{{ 6.3.2
